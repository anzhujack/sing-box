...
func strategyRoundRobin(g *LoadBalanceGroup) strategyFn {
	...
	return func(metadata *adapter.InboundContext, touch bool, matcher outboundMatcher) adapter.Outbound {
		candidates := g.getAlive()
		if len(candidates) == 0 {
			snap := g.state.Load()
			if snap == nil || len(snap.outbounds) == 0 {
				return nil
			}
			candidates = snap.outbounds
		}
		current := idx.Load()
		selected := candidates[int(current)%len(candidates)]
		if matcher != nil && !matcher(selected) {
			return nil
		}
		if touch {
			idx.Add(1)
...
func strategyStickySessions(g *LoadBalanceGroup, lruSize uint32) strategyFn {
	return strategyStickySessionsWithIndex(g, lruSize, func(key uint64, length int) int {
		return int(jumpHash(key, int32(length)))
	})
}

func strategyStickySessionsWithIndex(g *LoadBalanceGroup, lruSize uint32, selectIndex func(key uint64, length int) int) strategyFn {
	// LRU stores outbound TAG (string), not index — survives provider updates.
	lruCache := common.Must1(freelru.New[uint64, string](lruSize, maphash.NewHasher[uint64]().Hash32, true))
	...
	return func(metadata *adapter.InboundContext, touch bool, matcher outboundMatcher) adapter.Outbound {
		...
		if cachedTag, has := lruCache.Get(key); has {
			if selected, ok := snap.outboundByTag[cachedTag]; ok && g.IsAlive(selected) {
				if matcher != nil && !matcher(selected) {
					return nil
				}
				return selected
		...
		var selected adapter.Outbound
		if keyStr != "" {
			start := selectIndex(key, len(all))
			for i := 0; i < len(all); i++ {
				candidate := all[(start+i)%len(all)]
				if g.IsAlive(candidate) {
					selected = candidate
					break
				}
			}
		...
		if selected == nil {
			...
			// Store before matcher evaluation so an ineligible L3 pre-match is
			// replayed by the following L4 selection instead of consuming a new
			// sticky choice.
