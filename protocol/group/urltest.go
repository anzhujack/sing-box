...
func (s *URLTest) Selected() string { return "" }
...
func (s *URLTest) SelectOutbound(tag string) bool {
	...
	for _, outbound := range s.group.outbounds {
		if outbound.Tag() == tag {
			if common.Contains(outbound.Network(), N.NetworkTCP) {
				s.group.selectedOutboundTCP = outbound
			}
			if common.Contains(outbound.Network(), N.NetworkUDP) {
				s.group.selectedOutboundUDP = outbound
			}
			return true
		}
	}
