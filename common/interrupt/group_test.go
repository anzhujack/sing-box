...
func TestGroupInterruptDoesNotBlockNewConnDuringUnderlyingClose(t *testing.T) {
	...
	group.NewConn(blocker, false, false)
	...
	go func() {
		registered <- group.NewConn(&noOpConn{}, false, false)
	}()
...
func TestConnCloseDoesNotBlockNewConnDuringUnderlyingClose(t *testing.T) {
	group := NewGroup()
	blocker := newBlockingCloseConn()
	wrapped := group.NewConn(blocker, false, false)
	t.Cleanup(blocker.unblock)
...
func TestPacketConnCloseDoesNotBlockNewPacketConnDuringUnderlyingClose(t *testing.T) {
	group := NewGroup()
	blocker := newBlockingCloseConn()
	wrapped := group.NewPacketConn(&blockingClosePacketConn{blockingCloseConn: blocker}, false, false)
	t.Cleanup(blocker.unblock)
	...
	go func() {
		registered <- group.NewPacketConn(&noOpPacketConn{noOpConn: &noOpConn{}}, false, false)
	}()
...
func TestConnCloseIsIdempotentAndReturnsFirstError(t *testing.T) {
	...
	wrapped := group.NewConn(underlying, false, false)
...
func TestPacketConnCloseIsIdempotentAndReturnsFirstError(t *testing.T) {
	...
	underlying := newTrackingCloseConn(wantErr)
	wrapped := group.NewPacketConn(&trackingClosePacketConn{trackingCloseConn: underlying}, false, false)
	...
	testCases := []struct {
	...
	}{
		{
			...
			wrap: func(group *Group, underlying *trackingCloseConn) io.Closer {
				return group.NewConn(underlying, false, false)
			},
		},
		{
			name: "PacketConn",
			wrap: func(group *Group, underlying *trackingCloseConn) io.Closer {
				return group.NewPacketConn(&trackingClosePacketConn{trackingCloseConn: underlying}, false, false)
			},
		},
	}
...
func TestGroupInterruptPreservesConnectionFiltering(t *testing.T) {
	...
	wrappedInternal := group.NewConn(internal, false, false)
	wrappedExternal := group.NewConn(external, true, false)
